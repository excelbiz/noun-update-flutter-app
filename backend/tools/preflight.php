...
try{
 ...
 $store->transactional(['summary_users','summary_transactions','payment_intents','wallet_credit_receipts','user_summaries','nu_app_sessions','nu_app_rate_limits','nu_app_quotes','nu_app_orders','nu_app_order_items','nu_app_downloads']);
 foreach(['news_upload'=>['id','title','message','ref_id'], 'guides_upload'=>['id','title','message','ref_id'],
  'scholarship_upload'=>['id','title','message','ref_id'],'files'=>['id','name','price','file_path']] as $table=>$required){
 ...
 foreach(['config.php','paystack.php','paystack_webhook.php'] as $name)if(!is_file($root.'/course/'.$name))throw new RuntimeException('course/'.$name.' is missing');
