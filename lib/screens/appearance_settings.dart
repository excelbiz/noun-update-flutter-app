...
class _AppearanceSettingsState extends State<AppearanceSettings> {
  ...
  String? connection;
  Future<void> save({ThemeMode? mode,String? font,String? accent}) async {
    if(saving)return;setState(()=>saving=true);
    try{await Appearance.instance.change(mode:mode,font:font,accent:accent);}catch(_){if(mounted)nuMessage(context,'Your preference could not be saved. Please try again.');}
  ...
  @override Widget build(BuildContext context)=>ListenableBuilder(listenable:Appearance.instance,builder:(context,_){final a=Appearance.instance;return NuPage(title:'Settings',child:ListView(padding:const EdgeInsets.all(18),children:[
    ...
    NuPanel(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('Display mode',style:TextStyle(fontWeight:FontWeight.w700)),const SizedBox(height:8),Wrap(spacing:8,runSpacing:4,children:[for(final m in ThemeMode.values)ChoiceChip(key:ValueKey('mode-${m.name}'),label:Text(switch(m){ThemeMode.system=>'System',ThemeMode.light=>'Light',ThemeMode.dark=>'Dark'}),selected:a.mode==m,onSelected:saving?null:(_)=>save(mode:m))])])),
    ...
    TextButton(onPressed:saving?null:()=>save(mode:ThemeMode.system,font:'Modern sans',accent:'Emerald'),child:const Text('Restore default appearance')),
