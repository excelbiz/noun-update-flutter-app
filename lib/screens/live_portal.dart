...
class _LivePortalState extends State<LivePortal> with WidgetsBindingObserver {
 ...
 Widget _grid(List<Map<String,dynamic>> entries,{bool compact=false})=>LayoutBuilder(builder:(context,box){
  ...
  return GridView.builder(shrinkWrap:true,physics:const NeverScrollableScrollPhysics(),itemCount:entries.length,
   gridDelegate:SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:columns,crossAxisSpacing:compact?8:10,mainAxisSpacing:12,mainAxisExtent:(compact?92:154)+math.max(0,scale-1)*70),
   itemBuilder:(c,i){final s=entries[i],id='${s['id']}';return Material(color:compact?Colors.transparent:Color.lerp(Colors.white,serviceColour(id),.055),borderRadius:BorderRadius.circular(16),child:InkWell(borderRadius:BorderRadius.circular(16),onTap:()=>_service(s),child:Padding(padding:EdgeInsets.all(compact?2:10),child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[GlossIcon(serviceIcon(id),color:serviceColour(id),size:compact?46:54),const SizedBox(height:10),Text(serviceLabel(id,'${s['label']}'),textAlign:TextAlign.center,maxLines:3,overflow:TextOverflow.ellipsis,style:TextStyle(fontSize:compact?10:12,fontWeight:FontWeight.w800,color:const Color(0xff14251c),height:1.15)),if(!compact)...[const SizedBox(height:5),Text(serviceCaption(id),textAlign:TextAlign.center,maxLines:3,overflow:TextOverflow.ellipsis,style:const TextStyle(fontSize:10,height:1.25))]]))));});
 });
 Widget _home()=>ListView(key:const PageStorageKey('home'),padding:const EdgeInsets.fromLTRB(16,18,16,28),children:[
  TextField(readOnly:true,onTap:()=>setState(()=>tab=2),style:const TextStyle(fontSize:12),decoration:const InputDecoration(isDense:true,contentPadding:EdgeInsets.all(12),hintText:'Search courses, updates, resources…',prefixIcon:Icon(Icons.search,size:20))),
  NuTitle('Welcome${profile==null?'':', ${profile!['name']}'} 👋',subtitle:'What would you like to achieve today?'),
  AcademicOverview(api:api),
  const NuTitle('Quick access'),
 ...
 Widget _study()=>ListView(key:const PageStorageKey('study'),padding:const EdgeInsets.all(16),children:[const NuTitle('Study smarter',subtitle:'Your courses, notes and revision in one place.'),const GreenBanner(title:'A little progress.\nEvery day.',text:'Open your course to read modules, save notes and track your progress.',icon:Icons.menu_book_rounded),const NuTitle('Your study library'),_grid(services.where((s)=>['courses','course-summary','exam-summary'].contains(s['id'])).toList()),NuPanel(child:ListTile(leading:const GlossIcon(Icons.timer_rounded,size:45),title:const Text('Pomodoro focus'),subtitle:const Text('Make time for one focused session'),onTap:()=>pushNu(context,const FocusTimer())))]);
 Widget _tools(){const priority=['fees','cgpa-calculator','personalized-timetable','result','courses','calendar','marketplace','mock','news'];final ordered=[for(final id in priority)...services.where((s)=>s['id']==id).take(1),...services.where((s)=>!priority.contains(s['id']))];final rows=ordered.where((s)=>'${s['label']} ${s['group']}'.toLowerCase().contains(search.toLowerCase())).toList();return ListView(key:const PageStorageKey('tools'),padding:const EdgeInsets.all(16),children:[const NuPanel(padding:0,child:ListTile(contentPadding:EdgeInsets.zero,leading:GlossIcon(Icons.handyman_rounded,size:48),title:Text('Tools',style:TextStyle(fontSize:22,fontWeight:FontWeight.w800,color:nuDeep)),subtitle:Text('Everything you need to stay on track',style:TextStyle(fontSize:12)))),const LearningBanner(),const NuTitle('Academic Tools',subtitle:'Quick access to key tools for your academic journey'),TextField(onChanged:(v)=>setState(()=>search=v),decoration:const InputDecoration(prefixIcon:Icon(Icons.search),hintText:'Search tools, summaries and more')),const SizedBox(height:20),_grid(rows),if(rows.isEmpty)const Text('No tools match your search.')]);}
 ...
 Widget _profile()=>ListView(key:const PageStorageKey('profile'),padding:const EdgeInsets.all(16),children:[
