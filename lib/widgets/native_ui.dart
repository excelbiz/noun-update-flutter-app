...
class CentralWalletCard extends StatelessWidget {
 ...
 @override Widget build(BuildContext context)=>Container(padding:const EdgeInsets.all(14),decoration:BoxDecoration(borderRadius:BorderRadius.circular(17),gradient:const LinearGradient(colors:[Color(0xff003f2e),Color(0xff007146)],begin:Alignment.topLeft,end:Alignment.bottomRight)),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
  ...
  const SizedBox(height:14),Wrap(spacing:7,runSpacing:6,children:[FilledButton.icon(style:FilledButton.styleFrom(backgroundColor:nuGold,foregroundColor:nuDeep,minimumSize:const Size(0,36),padding:const EdgeInsets.symmetric(horizontal:10),textStyle:const TextStyle(fontSize:11,fontWeight:FontWeight.w800)),onPressed:onFund,icon:const Icon(Icons.add_circle,size:16),label:const Text('Add funds')),OutlinedButton.icon(style:OutlinedButton.styleFrom(foregroundColor:Colors.white,side:const BorderSide(color:Color(0xff39a680)),minimumSize:const Size(0,36),padding:const EdgeInsets.symmetric(horizontal:10),textStyle:const TextStyle(fontSize:11)),onPressed:onHistory,icon:const Icon(Icons.receipt_long,size:16),label:const Text('History')),TextButton(style:TextButton.styleFrom(foregroundColor:Colors.white,textStyle:const TextStyle(fontSize:11)),onPressed:onTools,child:const Text('Use with tools →'))]),
 ]));
}
